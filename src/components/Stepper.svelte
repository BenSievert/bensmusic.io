	...
	<div>
		<div class="mb-3 border-b border-gray-200 pb-1">
			<button
				...
				class="text-primary-dark disabled:text-primary hover:text-primary mr-6 cursor-pointer text-lg disabled:cursor-default"
				>Previous</button
			>
			<button
				...
				class="text-primary-dark disabled:text-primary hover:text-primary cursor-pointer text-lg disabled:cursor-default"
				>Next</button
			>
		</div>
		...
		<div>
			{#each steps[selectedStep].subSteps as subStep, i}
				<div
					class="{i <= selectedSubStep
						? `opacity-100`
						: `opacity-0`} mb-2 text-sm transition-opacity duration-300 ease-in md:text-base"
				>
