	...
	<Section theme="secondary">
		{#if session}
			...
			<div class="visible md:invisible absolute grid grid-cols-1 content-center mt-2 w-full text-sm">
				{#each songs as song}
					<a class="shadow rounded-md py-3 flex justify-between px-4 items-center gap-8 mb-8 border border-gray-200" href="/song-bird/{song.id}">
						<div class="w-full">
						{#each headers.slice(0,-1) as [header, key]}
							<div class="flex justify-between gap-2 mb-2 items-center">
								<div class="text-primary-dark font-bold flex-1">{header}</div>
								<div class="text-left flex-1">{song[key]}</div>
							</div>
						{/each}
						</div>
